...
- **Render a location page per saved location (combined atlas).** *(Bug → feature.)* Today the render pipeline is **single-mode**: a project with an extent renders **only** the bbox grid and drops every saved location; a project with no extent renders **only the first** location. `renderAtlas` (`packages/render-cli/src/render.ts`) takes one `mode: "bbox" | "location"`, and `HttpRenderWorkerClient.ToWirePayload` picks bbox-or-first-location. The fix is a **combined render mode**: build the extent grid (if any) **and** append a fixed-scale `buildLocationPage` for *each* saved location into one `AtlasContract`, then render all panels. The C# `RenderService` already resolves and forwards both `Extent` and `Locations` to the worker — the drop happens only in the TS engine + the wire mapping, so this is contained to `atlas-core`/`render-cli` + `HttpRenderWorkerClient` + worker validation (+ tests). Respect the existing `MAX_ATLAS_PAGES` cap over the *combined* page count. *(Medium; the headline gap behind "only the box generates.")*
- **Per-location scale level (zoom in for small areas).** Let each saved important location carry its **own** scale preset (overriding the project scale) so a small town or a country house can render at a much more zoomed-in scale (e.g. 1:24,000 or finer) while a regional stop stays coarse — instead of every location page sharing the one project-wide scale. Builds directly on [Standard Scale Presets](#standard-scale-presets-feature) (the scale engine is already per-page) and pairs with the combined-atlas item above: add a nullable per-location `scalePresetId` on the Stage 2C `ImportantLocation` (round-tripped through the API + web location editor), and have the combined render fall back to the project scale when a location doesn't set one. *(Small–medium; data-model + UI + render wiring, no new geometry math.)*
