...
**Next (decided): the map-generation options below come *before* any more tier work (Level 4 is parked).** The near-term build order is the [Planned Enhancements](#planned-enhancements--locations-extent--route-atlases-user-requested-post-phase-c) — **per-location zoom/scale first**, then box→location, locations→bbox, CSV import, and the route atlas. After those: Stage 6 (landmarks + simple routes) → Stage 7 (PMTiles offline packages + optional self-hosted tile server) → Stage 9 (MVP polish: geocode search, project rename/duplicate, error UX). Stage 8 (QuestPDF) stays conditional; Stages 10–11 are post-MVP.
...
- **▶ NEXT — Per-location zoom/scale level (zoom in for small areas).** *(User-prioritized: "especially the zoom levels for location maps.")* Let each saved important location carry its **own** scale preset overriding the project scale, so a small town or a country house renders much more zoomed-in (e.g. 1:24,000 or finer) while a regional stop stays coarse — instead of every location page sharing the one project-wide scale. The scale/projection engine is already per-page, so the geometry is free; the work is the **page-scale plumbing**. **Design note (from the Stage 6B converge):** `AtlasContract` currently carries a *single* top-level `scale`, and the scale bar + USNG grid interval derive from it — so a true per-location zoom needs **per-page scale** moved onto `AtlasPage` (or a per-page scale map) and threaded into the `pdf-client` furniture (scale bar, grid) and `validateAtlas`, not just an input field. Plan: (1) add an optional per-page `scale` to `AtlasPage` in `atlas-core` (fallback to contract scale), (2) make `buildLocationPage`/the furniture read it, (3) add a nullable `scalePresetId` to the Stage 2C `ImportantLocation` (API + web location editor), (4) the combined render passes each location's scale through. *(Small–medium engine change; this is the next build.)*
